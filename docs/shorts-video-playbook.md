...
4. 이미지는 장면을 설명하는 역할이다.
   - 팬더가 칠판 앞에서 강의하는 느낌보다, 남자 내레이션에 맞춰 화면/차트/ETF 선택지를 자연스럽게 설명하는 느낌을 우선한다.
...
- 얼굴 고정 요소:
  ...
  - 둥근 검은 안경
  - 따뜻한 눈과 작은 미소
- 복장과 제스처는 자연스럽게 바꿀 수 있다.
  - 스마트캐주얼 재킷, 셔츠, 니트, 카디건
  - 태블릿을 들거나 차트를 가볍게 가리키는 손짓
  - 책상/스튜디오/금융 화면 앞에서 설명하는 구도
- 피해야 할 것:
  - 칠판 앞 고정 선생님 구도
  - 과장된 교수 복장
...
- 문제: 팬더 얼굴이 장면마다 달라질 수 있다.
  - 해결: visual prompt 생성기와 ChatGPT/OpenAI 이미지 프롬프트 export에 팬더 얼굴 고정 문구를 반복 주입한다.
